...
function openLeftMenu() {
    ...
    var activeChannel = document.querySelectorAll('._channels_group:not(.hidden) .ch-item.current-item')[0];
    activeChannel.classList.add('item-active');
    activeChannel.scrollIntoView();
    //stalker.hideMainLoader();
}
...
function openCategories() {
    ...
    elems = document.querySelectorAll(".categories-container, .channels-list-container");
    for (var i = 0; i< elems.length; i++) {
        elems[i].classList.remove("hidden");
    }
...
function closeCategories() {
    ...
    elem = document.querySelectorAll(".main-menu-header .arrow-container__arrow-icon")[0];
    elem.classList.add("mirror-vertical");
    document.getElementById('main-menu').classList.remove("open-categories");
    setWidth("main-menu", 30);
    ...
    for (var i = 0; i < categoriesList.length; i++) {
        if (categoriesList[i].classList.contains('item-active')) {
            ...
            if (i != 0) {
                ...
                if (!categoriesList[categoriesList.length-1].classList.contains('category_blocked')) {
                    categoryListTimer = setTimeout(function () {
                        adapter.openChoosenCategoryList(categoriesList[categoriesList.length - 1]);
                    }, 500);
                }
                else {                                  //если категория блокед то проверяем или trusted session позволяет отрисовать список заблок. каналов
                    stalker.openBlockedList();
                }
                break;
            }
        }
    ...
    for (var i = 0; i < categoriesList.length; i++) {
        if (categoriesList[i].classList.contains('item-active')) {
            ...
            if (i == (categoriesList.length - 1)) {     // если переходим на первую категорию с последней
                ...
                categoriesList[i + 1].classList.add('item-active');
                if (!categoriesList[i + 1].classList.contains('category_blocked')) {    //если не категория Блокед то отрисовываем список каналов
                    categoryListTimer = setTimeout(function () {
                        adapter.openChoosenCategoryList(categoriesList[i + 1]);
                    }, 500);
                }
...
Player.prototype.prevChannelInList = function () {
    ...
    // var elems = document.getElementsByClassName("ch-item");
    // for (i=0; i<elems.length; i++) {
    //     elems[i].classList.remove("ch-item_active", "item-active");
    // }
    document.querySelector(".ch-item.ch-item_active.item-active").classList.remove("ch-item_active", "item-active");
    var prevChannel = currentChannel.previousElementSibling;
    if (prevChannel.classList.contains("ch-item") && prevChannel.getAttribute("_cid")) {
...
Player.prototype.nextChannelInList = function () {
    ...
    if (currentChannel.nextElementSibling) {
        ...
        if (nextChannel.classList.contains("ch-item")) {
...
//for next-channel/prev-channel button
...
window.onload = function() {
    ...
    Stalker.prototype.prevChannelInList = function () {
        ...
        var activeChannel = document.querySelector('.ch-item.item-active');
        //this.channelMouseOver(activeChannel);
    };
...
//листаем список каналов вниз
    ...
    Stalker.prototype.channelMouseOver = function (self) {
        var oldActiveChannels = channelGroupsContainer.querySelectorAll('.ch-item.item-active');
        removeActiveClassFrom(oldActiveChannels);
        self.classList.add('ch-item_active', 'item-active');
    };
    ...
    //смотреть Epg
    Stalker.prototype.watchEpg = function () {
        var self = this;
        var selectedChannel = channelGroupsContainer.querySelector('.ch-item.item-active');
        activeChannel = selectedChannel;
        selected_ch_id = selectedChannel.getAttribute('_id');
    ...
    Stalker.prototype.pushToBlocked = function (ch_id, btn) {
        console.log('before PushToBlocked');
    ...
    Stalker.prototype.showMainLoader = function () {
        //mainLoader.classList.remove('hidden');
    };
    Stalker.prototype.hideMainLoader = function () {
